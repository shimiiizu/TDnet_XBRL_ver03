...
def get_pl_data(code):
    ...
    try:
        ...
        # デバッグ: 元データを出力
        print(f"\n=== Code {code} の元データ ===")
        for item in data[:10]:  # 最初の10件
            print(f"FY={item.get('fiscalYear')}, Period={item.get('period')}, NetSales={item.get('netSales')}")

        converted_data = convert_to_quarterly_from_period(data)

        print(f"\n=== 変換後のデータ ===")
        for item in converted_data[:10]:  # 最初の10件
            print(f"FY={item.get('_fiscalYear')}, Q={item.get('_quarter')}, NetSales={item.get('netSales')}")
